...
app.get("/", (req, res) => {
  ...
  <head>
    ...
    <style>
      body { font-family: "IBM Plex Sans", Arial, sans-serif; background-color: #f4f4f4; margin: 0; padding: 0; }
      .auth-container { width: 400px; background: #fff; border-radius: 12px; padding: 40px 30px; text-align: center; box-shadow: 0px 4px 15px rgba(0,0,0,0.1); position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); }
      ...
      input[type="email"], input[type="password"] { width: 100%; padding: 10px 12px; border: 1px solid #c6c6c6; border-radius: 6px; font-size: 14px; transition: border 0.2s; }
      input:focus { border-color: #0f62fe; outline: none; }
      .btn { width: 100%; padding: 12px; background-color: #0f62fe; color: #fff; border: none; border-radius: 6px; font-size: 16px; font-weight: 600; cursor: pointer; transition: background 0.2s; }
      .btn:hover { background-color: #0353e9; }
      .link { display: block; margin-top: 15px; font-size: 14px; color: #0f62fe; text-decoration: none; }
      ...
      .header { background-color: #0f62fe; color: #fff; padding: 20px 40px; font-size: 20px; font-weight: 600; }
      h1 { font-size: 28px; color: #161616; margin-top: 30px; }
      p { font-size: 16px; color: #393939; margin-top: 10px; }
      .btn-logout { margin-top: 30px; background-color: #da1e28; color: #fff; border: none; border-radius: 6px; padding: 12px 25px; font-size: 16px; cursor: pointer; transition: background 0.2s; }
...
// ==== Start Server ====
const PORT = 5000;
app.listen(PORT, () => console.log(`IBM Auth Dashboard running at http://localhost:${PORT}`));
